...
## Cover Gallery
For a cover gallery of Lux, see [[Lux (book)/Covers\|/Covers]].
## Development
...
### Prose Version
In 2021, [[Peter Ahlstrom\|Peter Ahlstrom]] believed that a prose version of the novel would certainly be released, and Brandon stated that there would be an eBook and eventually a print version. However, Peter has since mentioned that there are several hurdles to overcome. The [[Dragonsteel\|Dragonsteel]] team has been focused on Brandon's [[Cosmere\|cosmere]] works, so finding the resources to work on a prose version of *Lux* has been difficult. Peter noted that it can be difficult to maintain good pacing while moving from audio to prose, and *Lux* also has some continuity issues with the other *Reckoners* works that need to be fixed. As of 2025, the prose version is in limbo.

### Sequels
Two sequels to *Lux* were considered. Although *Lux* was considered to be a success, there are no firm plans to move forward with the sequels, presumably due to the prioritization of other projects. Brandon has referred to *The Reckoners* as "finished", with only a small chance that it could receive another sequel someday. If one or more sequels are written, it would increase the chances of a prose version of *Lux*.
